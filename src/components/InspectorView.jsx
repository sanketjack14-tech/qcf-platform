...
export default function InspectorView({ 
  ...
  const activeDomainStatements = activeStatements.filter(s => s.domainNumber === selectedDomainId);
  const activeStatement = activeStatements.find(s => s.id === selectedStmtId) || activeStatements[0] || statements[0];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="bg-[#16362B] text-white rounded-3xl p-6 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border border-emerald-800">
        ...
        {/* Assigned School Selector Dropdown & Download Report Action */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
          <div className="bg-emerald-950/80 border border-emerald-800 p-2.5 rounded-2xl">
            <label className="block text-[10px] font-bold text-emerald-300 uppercase tracking-wider mb-1 px-1">
              Select School To Audit:
            </label>
            <select
          ...
          <button
            ...
            className="px-5 py-3 bg-emerald-500 hover:bg-emerald-400 text-emerald-950 font-black text-xs rounded-2xl shadow-lg flex items-center justify-center gap-2 transition-all shrink-0 cursor-pointer border border-emerald-300"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-white rounded-2xl border border-emerald-100 p-4 shadow-xs">
            ...
            <div className="space-y-1.5">
              {QCF_DOMAINS.map(d => {
                ...
                const domainStmtsCount = activeStatements.filter(s => s.domainNumber === d.id).length;
                return (
                  <button
                    key={d.id}
                    onClick={() => {
                      setSelectedDomainId(d.id);
                      const firstStmt = activeStatements.find(s => s.domainNumber === d.id);
          ...
          <div className="bg-white rounded-2xl border border-emerald-100 p-4 shadow-xs">
            <h3 className="text-xs font-bold text-gray-800 uppercase tracking-wider mb-3">
              2. Domain {selectedDomainId} Standards List ({activeDomainStatements.length})
            </h3>
            <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
              {activeDomainStatements.map(stmt => {
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {activeStatement ? (
            ...
            <div className="bg-white rounded-2xl border border-emerald-100 p-8 text-center text-xs text-gray-500">
              Select a standard from the list to audit evidence and score.
            </div>
          )}
          ...
          {activeStatement && (
            <div className="bg-[#16362B] text-white rounded-2xl p-6 shadow-md border border-emerald-800 space-y-5">
              ...
              <div className="flex justify-between items-center pt-2">
                <button
                  onClick={() => setIsReportModalOpen(true)}
                  className="px-4 py-2 bg-emerald-900 hover:bg-emerald-800 border border-emerald-700 text-emerald-100 font-bold text-xs rounded-xl flex items-center gap-2 transition-colors cursor-pointer"
                >
      ...
      {/* Official Downloadable Report Modal */}
